...
int SearchCache(int key)
{
	...
	while (temp != NULL) {
		...
		if (temp->key == key) {
			// Shift all values before the found value to the right.
			while (temp != head) {
...
// Function to perform LRU operations
void LRUrefer(int arr[], int n)
{

	// Iterating through the elements so that LRU operation can take place
	for (int i = 0; i < n; ++i) {
...
int main(void)
{
	...
	// The Numbers to be added in LRU List.
	//int arr[] = { 1, 2, 3, 4, 5, 2, 10, 7, 11, 1 };
	int arr[] = { 2, 3, 2, 1, 5, 2, 4, 5, 3, 2, 5, 2 };
	
	// Number of elements to be added in LRU List.
